...
Patterns for structuring multi-step workflows and decision logic inside a skill.
...
Guide the agent through decision points with clear branching:
...
## Ratchet Loops (Iteration)

For skills that optimize an artifact (prompts, configs, other skills), combine with the `auto-eval` skill's ratchet pattern:

```markdown
1. Baseline — score the current artifact
2. Propose a change (smallest diff that addresses a specific weakness)
3. Commit
4. Re-score
5. If score ≥ baseline → keep; else `git revert HEAD`
6. Repeat until plateau or gate is cleared
```

Never overwrite history with `reset --hard`; `git revert HEAD` preserves the failed-attempt trail for diagnostics.
